...
# Introduction 
Developmental psychology is facing a dilemma: many research questions are questions about individual differences, yet, there is a lack of tasks to reliably measure these individual differences. Since children spend a large amount of their daily lives interacting with and learning from others in social contexts, many studies investigate the emergence and driving factors of social-cognitive abilities. Under social cognition, we understand the ability to explain and predict the behavior of social agents by representing and reasoning about their perspectives, knowledge, intentions, beliefs, and preferences. 

Traditional tasks of social cognition (e.g., false belief change-of-location tasks) frequently build upon dichotomous outcome variables (resulting in high chance levels), small sample sizes and low numbers of trials per subject. 

With these procedures, researchers could systematically track change on a group level and capture the average age at which children succeed in a given task that relies on a certain social-cognitive ability. However, poor psychometric properties may conceal relations between different aspects of cognition and also obscure developmental change.

If we want to answer questions about the co-development of social-cognitive abilities and the individual’s pace of mastering developmental milestones, we need to take variation between children seriously – both methodologically and theoretically. We argue that a systematic individual differences perspective on social-cognitive development is needed.
...
A recent paper by @hedge2018reliability argued that cognitive tasks often become well-established by displaying robust effects across studies. The authors could show that this often does not emerge due to high measurement variance but rather low between-subject variability, concluding that popular cognitive tasks may reliably measure group differences but not individual differences. In a similar vein, @pronk2021methods reasoned that accurate reliability estimates are needed to judge how well a certain cognitive test is suited to draw inferences about individuals.

From previous studies that tried to relate several theoretically connected constructs to each other, we see mostly failed attempts. For example, @sodian2016understanding studied the relation between several social cognition measures. Against expectation, they could not find correlations between some theoretically connected constructs (e.g. implicit false belief  and moral false belief understanding). Interestingly, nearly all measured variables were associated with their verbal IQ test. This shows that we are more likely to find associations between constructs when measures are actually designed to capture variation between individuals - similarly to how IQ tests are constructed. Therefore, it is conceivable that we are simply not able to capture existing underlying relationships with our existing tools - since they were mostly not created to quantify individual differences. 
...
To estimate the effect of inter-individual differences, we need a large sample size since since some of the effects that we are looking for might be small. Since data collection with families requires a lot of organizational effort, we want to enable data collection in-person with supervision as well as remotely without supervision. A standardized, easily accessible test procedure helps us to enable data collection at scale.

Another goal in creating new tasks should be to focus on the face value: we want to measure the underlying social-cognitive ability as straight-forward and directly as possible. Importantly, we can assess whether the task shows external validity by assessing the relationship with related constructs and factors of children's social surrounding. 

In our first novel task, we concentrated on the essential ability that is involved in many social-cognitive reasoning processes: gaze cue understanding - the ability to locate and use the attentional focus of an agent. The first component is often termed gaze following - turning ones eyes in the same direction as the gaze of another agent. By acting as a 'front end ability' [@brooks2005development, p. 535], following an agent's gaze provides insights into their intentions, thoughts, and feelings. In our definition, gaze cue understanding goes one step further by including the *acting* on the gaze-cued location. 

In many previous gaze following studies, the eye movement of an agent was accompanied by a congruent head movement - thus, making it hard to disentangle which information the children rely on. In our task, we control the eye movement alone to pinpoint and break down children's abilities as detailed as possible. 

By implementing studies with open versus closed eyes conditions and head versus eye movement alone, researchers found that infants start gaze following in the narrow sense at the end of their first year of life (e.g., @brooks2005development). 
It is, however, unclear when and how children start to use these cues to guide their own active behavior. 
