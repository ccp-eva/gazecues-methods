...
mDataAgeHedge <- vali %>% 
  ...
  select(subjID, age, ageInMonths, datacollection, symmetricPosition, targetPosition, clickDistInBalloons)
...
nd_hedge <- mDataAgeHedge %>% 
  select(-c(clickDistInBalloons, symmetricPosition, targetPosition))
  
# extract model predictions for mock data
pred_hedge <- fitted(mAgeHedge, newdata = nd_hedge, re_formula = NA) %>% as_tibble() %>% bind_cols(mDataAgeHedge)
...
ggsave("../figures/gafo_hedge_variation_GLMM.png", width = 7, height = 4)
```
...
mDataAgeBox <- vali %>% 
  ...
  select(subjID, age, ageInMonths, datacollection, symmetricPosition, targetPosition, correctBox)
...
```{r GLMM_plot_box}
nd_box <- mDataAgeBox %>% 
  select(-c(correctBox, symmetricPosition, targetPosition))
...
ggsave("../figures/gafo_box_variation_GLMM.png", width = 7, height = 4)
```
...
splitscores_hedge_random_kids <- by_split(
  ...
  replace = TRUE,
  split_p = 1,
  ncores = 7, # setting ncores equally to something less than detectCores()
)

hedge_random_kids <- split_coefs(splitscores_hedge_random_kids, cor)
uniq_reps <- unique(splitscores_hedge_random_kids$replication)

parallel_split_ci <- function(r) {
        return(split_ci(filter(splitscores_hedge_random_kids, replication==r), cor))
}

hedge_random_kids_ci <- mclapply(uniq_reps, parallel_split_ci, mc.cores=48)
# Darn R
...
splithalf_data$CI_lower = numeric(nrow(splithalf_data))
splithalf_data$CI_upper = numeric(nrow(splithalf_data))
for(i in 1:nrow(splithalf_data)) {
  splithalf_data$CI_lower[i] <- hedge_random_kids_ci[[i]]$lims["0.025","bca"]
  splithalf_data$CI_upper[i] <- hedge_random_kids_ci[[i]]$lims["0.975","bca"]
}

print(splithalf_data)
# write_csv(splithalf_data, "splithalf_data.csv")
```
...
ggsave("../figures/gafo_box_retest_kids.png", width = 6, height = 4)
```
...
ggsave("../figures/gafo_results_GLMM.png", width = 6.5, height = 5, scale = 1.6, bg = "white")
```
...
ggarrange(hedge_variation_plot,
          ...
          nrow = 3, heights = c(0.625, 0.05, 0.325), labels = c("A", "", "")
)

ggsave("/Users/julia_prein/Downloads/gafo_poster.png", width = 6.5, height = 6, scale = 1.1, bg = "white")
```
...
n_distinct(mDataExtvali$subjID)
```
...
```{r extvali_savemodels}
write.csv(mExtvalinull, "../saves/extvali-mExtvalinull.csv", quote = F, row.names = F)
saveRDS(mExtvalinull, "../saves/extvali-mExtvalinull.rds")

write.csv(mExtvaliccAge, "../saves/extvali-mExtvaliccAge.csv", quote = F, row.names = F)
saveRDS(mExtvaliccAge, "../saves/extvali-mExtvaliccAge.rds")

write.csv(mExtvaliccHours, "../saves/extvali-mExtvaliccHours.csv", quote = F, row.names = F)
saveRDS(mExtvaliccHours, "../saves/extvali-mExtvaliccHours.rds")

write.csv(mExtvalihhTotal, "../saves/extvali-mExtvalihhTotal.csv", quote = F, row.names = F)
saveRDS(mExtvalihhTotal, "../saves/extvali-mExtvalihhTotal.rds")

write.csv(mExtvalihhSibs, "../saves/extvali-mExtvalihhSibs.csv", quote = F, row.names = F)
saveRDS(mExtvalihhSibs, "../saves/extvali-mExtvalihhSibs.rds")

write.csv(mExtvalisibsChildAged, "../saves/extvali-mExtvalisibsChildAged.csv", quote = F, row.names = F)
saveRDS(mExtvalisibsChildAged, "../saves/extvali-mExtvalisibsChildAged.rds")

write.csv(mExtvalisiblingsVarietyScoreP, "../saves/extvali-mExtvalisiblingsVarietyScoreP.csv", quote = F, row.names = F)
saveRDS(mExtvalisiblingsVarietyScoreP, "../saves/extvali-mExtvalisiblingsVarietyScoreP.rds")

write.csv(mExtvalisiblingsVarietyScoreW, "../saves/extvali-mExtvalisiblingsVarietyScoreW.csv", quote = F, row.names = F)
saveRDS(mExtvalisiblingsVarietyScoreW, "../saves/extvali-mExtvalisiblingsVarietyScoreW.rds")

write.csv(mExtvaliPE, "../saves/extvali-mExtvaliPE.csv", quote = F, row.names = F)
saveRDS(mExtvaliPE, "../saves/extvali-mExtvaliPE.rds")
```
...
extvali_results <- mExtvaliComparison %>%
  ...
  mutate(
    Predictor = recode(Predictor, 
                   "mExtvaliPE" = "Peer exposure index", 
                   "mExtvalisiblingsVarietyScoreW" = "Sibling variety score (Cassidy et al., 2005)", 
                   "mExtvalihhSibs" = "# Children in household aged 0-18", 
                   "mExtvaliccHours" = "Average hours spent in childcare per day",
                   "mExtvalinull" = "Null model", 
                   "mExtvalihhTotal" = "# Household members",
                   "mExtvaliccAge" = "Age of childcare entry", 
                   "mExtvalisiblingsVarietyScoreP" = "Sibling variety score (Peterson, 2000)", 
                   "mExtvalisibsChildAged" = "# Children in household aged 1-12",
                   )
  )
...
```{r extvali_posteriordraws}
extvali_draws <- bind_rows(
  mExtvaliccAge %>% gather_draws(b_ccAge),
  mExtvaliccHours %>% gather_draws(b_ccHours), 
  mExtvalihhTotal %>% gather_draws(b_hhTotal), 
  mExtvalihhSibs %>% gather_draws(b_hhSibs), 
  mExtvalisibsChildAged %>% gather_draws(b_sibsChildAged), 
  mExtvalisiblingsVarietyScoreP %>% gather_draws(b_siblingsVarietyScoreP), 
  mExtvalisiblingsVarietyScoreW %>% gather_draws(b_siblingsVarietyScoreW),
  mExtvaliPE %>% gather_draws(b_pe)
  ) %>% 
  mutate(
...
write.csv(extvali_draws, "../saves/extvali-draws.csv", quote = F, row.names = F)
saveRDS(extvali_draws, "../saves/extvali-draws.rds")
```

```{r extvali_plot}
ggplot(extvali_draws, aes(y = Predictor, x = .value, fill = stat(x) > 0)) +
 tidybayes::stat_halfeye(alpha = .8, .width = c(.95, .80), scale = 3) +
...
ggsave("../figures/extvali_results_all.png", width = 10, height = 6, scale = 0.7, bg = "white")

extvali_draws %>% 
  filter(
          Predictor == "Peer exposure index" ||
          Predictor == "Sibling variety score (Cassidy et al., 2005)" ||
          Predictor == "Number of children in household aged 0-18" ||
          Predictor == "Average hours spent in childcare per day", 
         ) %>% 
 ...
 tidybayes::stat_halfeye(alpha = .8, .width = c(.95, .80)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed") +
  scale_fill_manual(values = c("grey","#85AD00")) +
  scale_x_continuous(limits = c(-0.6, 1), breaks = seq(-0.5, 1, 0.25)) + 
  theme_minimal()

ggsave("../figures/extvali_results.png", width = 10, height = 6, scale = 0.7, bg = "white")
```
