...
hedge_variation_plot <- ggplot() +
  ...
  labs(x = "Age in years", y = "Average imprecision in target width", col = "Age", shape = "Data collection mode", alpha = "Data collection mode", lty = "Data collection mode") + 
...
hedge_intCon <- reli %>% 
  ...
  # group_by(subjID, targetPosition, sample) %>% 
  mutate(
    testhalf = ifelse(trialNr %% 2 == 0, "even_trials", "odd_trials")
  ) %>% 
  group_by(subjID, testhalf, sample) %>%
...
hedge_intCon_kids <- hedge_intCon %>% 
  ...
  stat_cor(method = "pearson", label.x = 0, label.y = 5, aes(label = paste(..r.label..)), size = 4, r.accuracy = 0.01, cor.coef.name = "r") + 
...
hedge_intCon_adults <- hedge_intCon %>% 
  ...
  geom_smooth(method = "lm", size = 0.5, alpha = 0.15, col = "darkgrey") +
  stat_cor(method = "pearson", label.x = 0, label.y = 2, aes(label = paste(..r.label..)), size = 4, r.accuracy = 0.01, cor.coef.name = "r") + 
...
box_intCon_kids <- box_intCon %>% 
  ...
  geom_point(col = "#006c66", size = 0.7) + 
  geom_smooth(method = "lm", size = 0.5, alpha = 0.15, col = "darkgrey") +
  stat_cor(method = "pearson", label.x = 0, label.y = 1, aes(label = paste(..r.label..)), size = 4, r.accuracy = 0.01, cor.coef.name = "r") + 
...
hedge_retest <- reli %>% 
  filter(studyversion == "hedge") %>% 
  group_by(subjID, reliday, sample) %>% 
  tidyboot_mean(column = abs(clickDistFromTargetCenterX)/160, na.rm = TRUE)
...
hedge_retest_kids <- hedge_retest %>% 
  ...
  geom_point(col = "#006c66", size = 0.7) + 
  geom_smooth(method = "lm", size = 0.5, alpha = 0.15, col = "darkgrey") +
  stat_cor(method = "pearson", label.x = 0, label.y = 4, aes(label = paste(..r.label..)), size = 4, r.accuracy = 0.01, cor.coef.name = "r") + 
