...
hedge_intCon_data <- reli %>% 
  filter(studyversion == "hedge" & reliday == "reliday1") %>% 
  mutate(
    # summarize bins, so that 9 + 10 = bin 5
    # bin = ceiling(as.numeric(targetPosition)/2) 
    
    # summarize so that 1 + 10 = bin 5
    symmetricBin = case_when(targetPosition == "5" | targetPosition == "6" ~ 1, 
...
splitscores_hedge_randomStrat_kids <- by_split(
  ...
  hedge_intCon_data %>% filter(sample == "kids") %>% pull(subjID),
  fn_score_hedge,
  stratification = hedge_intCon_data %>% filter(sample == "kids") %>% pull(symmetricBin),
  replications = 1,
  method = c("random"),
  replace = TRUE,
  split_p = 1,
...
splitscores_hedge_randomStrat_adults <- by_split(
  ...
  hedge_intCon_data %>% filter(sample == "adults") %>% pull(subjID),
  fn_score_hedge,
  stratification = hedge_intCon_data %>% filter(sample == "adults") %>% pull(symmetricBin),
  replications = 1,
  method = c("random"),
  replace = TRUE,
  split_p = 1,
