...
```{r orev_plot}
orev_plot <- ggplot(mDataOrev, aes(x = mean_orev, y = mean_tango)) +
  
  geom_point(aes(col = as.factor(ageInYears)), shape = 18, size = 2) +
  geom_smooth(method = "glm", size = 0.5, alpha = 0.15, col = "darkgrey") +
  stat_cor(method = "pearson", label.x = 0.9, label.y = 0.975, aes(label = paste(..r.label..)), size = 4, r.accuracy = 0.01, cor.coef.name = "r") + 
  
  scale_color_manual(values = c("#c6d325", "#00b1ea", "#29485d", "#9D3BBD", "#F71C8D"), labels = c("3-year-olds", "4-year-olds", "5-year-olds", "6-year-olds", "7-year-olds")) + 
  
  labs(x = "oREV score", y = "TANGO score", col = "Age in years") + 
