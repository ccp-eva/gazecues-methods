---
title             : "Supplements for the manuscript  'TANGO: A reliable, open-source, browser-based task to assess individual differences in gaze understanding in 3 to 5-year-old children and adults'"
...
```{r reli_agegroup, include=FALSE}
hedge_intCon_agegroup <- readRDS("../saves/tango-hedge-intCon-kids.rds") + 
  geom_point(aes(col = ageInYears), size = 1) + 
  facet_wrap(~ ageInYears)

hedge_retest_agegroup <- readRDS("../saves/tango-hedge-retest-kids.rds") + 
  geom_point(aes(col = ageInYears), size = 1) + 
  facet_wrap(~ ageInYears)

box_intCon_agegroup <- readRDS("../saves/tango-box-intCon-kids.rds") + 
  geom_point(aes(col = ageInYears), size = 1) + 
  facet_wrap(~ ageInYears)

box_retest_agegroup <- readRDS("../saves/tango-box-retest-kids.rds") + 
  geom_point(aes(col = ageInYears), size = 1) + 
  facet_wrap(~ ageInYears)

ggarrange(hedge_intCon_agegroup, 
          hedge_retest_agegroup,
          box_intCon_agegroup, 
          box_retest_agegroup, 
          ncol = 2,
          nrow = 2, 
...
ggsave("../figures/tango_supplements_reli_byagegroup.png", width = 9, height = 6)
```
...
orev_results <- readRDS("../saves/socenv_results.rds") %>% 
  mutate(across(c(WAIC, Weight, ELPD_DIFF, SE_ELPD_DIFF), as.numeric)) %>%
  mutate_if(is.numeric, round, 2)
...
levels(testtrials$`Current Country of Residence`)
```
...
adult_sample_countryn <- testtrials %>% 
  group_by(subjID) %>% 
  slice(1) %>% 
  group_by(`Current Country of Residence`, studytype) %>%
  summarise(n = n()) %>%
  arrange(studytype, -n) %>% 
  filter(!is.na(`Current Country of Residence`) & !is.na(studytype))
